...
static void di_release(struct gendisk *disk, fmode_t mode)
{
...
static int __exit di_of_remove(struct platform_device *odev)
{
	return di_do_remove(&odev->dev);
}

static void di_of_shutdown(struct platform_device *odev)
{
	di_do_shutdown(&odev->dev);
}
...
static int __init di_init_module(void)
{
	...
	return platform_driver_register(&di_of_driver);
}

static void __exit di_exit_module(void)
{
	platform_driver_unregister(&di_of_driver);
}
